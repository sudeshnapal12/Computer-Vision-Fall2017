...
def Perspective_warping(img1, img2, img3):
    ...
    # Calculate homography transformation between img2 and img1.
    (M, pts1, pts2, mask) = getTransform(img2, img1, 'homography')
    # Transform img2 to plane of img1.
    tmp1 = cv2.warpPerspective(img2, M, (img1.shape[1],img1.shape[0]), dst=img1.copy(), borderMode=cv2.BORDER_TRANSPARENT)
	
    #plt.imshow(tmp1, cmap='gray')
    #plt.show()
	
    # Calculate homography transformation between img3 and img1.
    (M, pts1, pts2, mask) = getTransform(img3, tmp1, 'homography')
    # Transform img3 to plane of transformed img1.
    tmp2 = cv2.warpPerspective(img3, M, (img1.shape[1],img1.shape[0]), dst=tmp1.copy(), borderMode=cv2.BORDER_TRANSPARENT)
...
def Bonus_perspective_warping(img1, img2, img3):
    ...
    # Create masks and add rom aroung img1.
    mask1 = np.ones(img1.shape)
    mask2 = np.ones(img2.shape)
    mask3 = np.ones(img3.shape)
    img1 = cv2.copyMakeBorder(img1,200,200,500,500, cv2.BORDER_CONSTANT)
    mask1 = cv2.copyMakeBorder(mask1,200,200,500,500,cv2.BORDER_CONSTANT)
    
    # Homography transform and project img2 into img1.
    (M, pts1, pts2, mask) = getTransform(img2, img1, 'homography')
    im2Warp = cv2.warpPerspective(img2, M, (img1.shape[1],img1.shape[0]), dst=img1.copy(), borderMode=cv2.BORDER_TRANSPARENT)
	
    # Homography transform and project img3 into transformed img1.
    (M, pts1, pts2, mask) = getTransform(img3, im2Warp, 'homography')
    im3Warp = cv2.warpPerspective(img3, M, (img1.shape[1],img1.shape[0]), dst=im2Warp.copy(), borderMode=cv2.BORDER_TRANSPARENT)
    mask3Warp = cv2.warpPerspective(mask3, M, (img1.shape[1],img1.shape[0]), dst=im2Warp.copy(), borderMode=cv2.BORDER_TRANSPARENT)
    m = np.ones(mask3Warp.shape, dtype='float32')
    m[mask3Warp!=0] = 0
	
    # Laplacian blending of projected images.
    out = Laplacian_Pyramid_Blending_with_mask(im2Warp, im3Warp, m, 4)
...
def Cylindrical_warping(img1, img2, img3):
    ...
    # Assuming same h,w and K for all the images.
    # Apply cylindrical warping to image1, image2, image3.
    h,w = img3.shape
    f = 400
    K = np.array([[f, 0, w/2], [0, f, h/2], [0, 0, 1]]) # mock calibration matrix
    imcyl3, mask3 = cylindricalWarpImage(img3, K)
    imcyl2, mask2 = cylindricalWarpImage(img2, K)
    imcyl1, mask1 = cylindricalWarpImage(img1, K)
	
    # Making some room around cylindrical_image1.
    imcyl1 = cv2.copyMakeBorder(img1,50,50,300,300, cv2.BORDER_CONSTANT)
	
    # Affine transformation between cylindrical_image3, cylindrical_image2 and cylindrical_image1.
    # Transform image2 and image3 to the plane of image1 and use alpha blending to remove black border.
    (M, pts1, pts2, mask) = getTransform(imcyl2, imcyl1, 'affine')
    im2Warp = cv2.warpAffine(imcyl2, M, (imcyl1.shape[1],imcyl1.shape[0]))
	
    (M1, pts1, pts2, mask) = getTransform(imcyl3, im2Warp, 'affine')
    im3Warp = cv2.warpAffine(imcyl3, M1, (imcyl1.shape[1],imcyl1.shape[0]))
    im3Warp_mask = cv2.warpAffine(mask3, M1, (imcyl1.shape[1],imcyl1.shape[0]))
    im3Warp_mask = im3Warp_mask/255 # Converting to binary mask
	
    # Alpha blending to remove black border.
    out = im2Warp*(1-im3Warp_mask) + im3Warp*im3Warp_mask

    #cv2.imshow('tmp', im2Warp)
    #cv2.imshow('tmp_mask', im3Warp_mask)
    #cv2.imshow('out', out)
    #cv2.waitKey(0)
...
def Bonus_cylindrical_warping(img1, img2, img3):
    ...
    m = np.ones(im3Warp_mask.shape, dtype='float32')
    m[im3Warp_mask!=0] = 0
    out = Laplacian_Pyramid_Blending_with_mask(im2Warp, im3Warp, m, 3)
...
if __name__ == '__main__':
    ...
    # Calculate RMSD for each output
    #im_ex = cv2.imread('example_output1.png', 0)
    #im_cy_ex = cv2.imread('example_output2.png', 0)
    #im = cv2.imread('output_homography.png', 0)
    #imb = cv2.imread('output_homography_lpb.png', 0)
    #im_cy = cv2.imread('output_cylindrical.png', 0)
    #imb_cy = cv2.imread('output_cylindrical_lpb.png', 0)
    #print "homography" , RMSD(1, im_ex, im)
    #print "homography_bonus" , RMSD(1, im_ex, imb)   
    #print "affine" , RMSD(2, im_cy_ex, im_cy)
    #print "affine_bonus" , RMSD(2, im_cy_ex, imb_cy)
